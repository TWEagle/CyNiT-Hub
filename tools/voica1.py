...
# !/usr/bin/env python3
"""
VOICA1 device certificaten-tool voor CyNiT Hub & Standalone.
- Standalone: python tools/voica1.py -> http://127.0.0.1:5445/voica1
- In Hub: master.register_tools(...) roept register_web_routes(app)

Functionaliteit:
- Engine-keuze: Python (cryptography) of OpenSSL
...
# Gebruik hub-layout (main_layout.py) i.p.v. cynit_* helpers
# -> levert header/topbar/menus/footer + /static/main.css en /static/main.js
from beheer.main_layout import render_page as hub_render_page


# =========================
# Logging (zorg voor zichtbare logs in PowerShell)
# =========================
...
# =========================
# Project/Config paths (robuust, werkt vanuit tools/ én projectroot)
# =========================
...
# Globals / defaults
# =========================
# Let op: ROOT_BASE_DIR kan via config/voica1.json worden overschreven.
...
# UI / state (hier niet meer gebruikt voor theming, behouden voor compat)
...
def generate_password(length: int) -> str:
    ...
    while True:
        ...
        # Constraint: niet starten/eindigen met symbool; minstens één symbool binnenin
...
def run_cmd(cmd: List[str], cwd: Optional[Path] = None) -> str:
    ...
    try:
        ...
        raise CommandError(
            ...
            f"Commando: {' '.join(cmd)}\n"
            "Fix opties:\n"
            " - Zet in config/voica1.json: openssl_bin naar het volledige pad "
            "(bv. C:\\openssl\\x64\\bin\\openssl.exe)\n"
...
def py_create_key_and_csr(base_dir: Path, cn: str, key_size: int) -> Tuple[Path, Path]:
    ...
    csr_path.write_bytes(csr.public_bytes(encoding=serialization.Encoding.PEM))
    return key_path, csr_path
...
# =========================
# CONTENT-ONLY TEMPLATE (Hub-layout verzorgt header/footer/css/js)
# =========================
CONTENT_TEMPLATE = r"""
<style>
/* VOICA1-specifiek (compact); rest komt uit /static/main.css */
...
.results-table th, .results-table td { border: 1px solid rgba(255,255,255,.15); padding: 6px 8px; }
.results-table th { background: rgba(255,255,255,.06); color: var(--muted, #9fb3b3); }
.muted { color: var(--muted, #9fb3b3); font-size: 0.92rem; }
...
.voica-progress-box {
  background: rgba(10,15,18,.92); border: 1px solid var(--border, rgba(255,255,255,.10)); padding: 20px 30px;
  border-radius: 12px; box-shadow: 0 0 20px rgba(0, 0, 0, 0.9); min-width: 280px; max-width: 520px; text-align: center;
}
.voica-progress-title { font-size: 1.1em; margin-bottom: 12px; color: var(--text, #e8f2f2); }
.voica-progress-bar-outer { width: 100%; height: 10px; background: #222; border-radius: 6px; overflow: hidden; margin-top: 6px; }
.voica-progress-bar-inner { width: 40%; height: 100%; background: var(--accent, #35e6df); border-radius: 6px; animation: voica-bar-move 1.0s linear infinite; }
...
<div class="voica-container">
  ...
  <div class="card">
    ...
    <form id="voica-form-generate" method="post" action="/voica1/generate">
      ...
      <div class="field-row">
        <button class="btn" type="submit">Stap 1: Genereer CSR(s)</button>
      </div>
    </form>
  </div>
  ...
  <div class="card">
    ...
    <form id="voica-form-process" method="post" action="/voica1/process">
      ...
      <div class="field-row">
        ...
        <small class="muted">Zelfde wachtwoord voor alle toestellen en ZIP (phones).</small>
      </div>

      <div class="field-row">
        <button class="btn" type="submit" {% if not step1_done %}disabled{% endif %}>Stap 2: Verwerk certificaten</button>
      </div>
    </form>
  </div>
  ...
  <div class="card">
    ...
    <div class="field-row">
      ...
      <button class="btn" type="button" onclick="copyText('txt_certmail')">Kopieer CERT mail</button>
    </div>
    <div class="field-row">
      <label>OTS mail</label>
      <textarea id="txt_ots" class="textarea-small">{{ ots_text }}</textarea>
      <button class="btn" type="button" onclick="copyText('txt_ots')">Kopieer OTS mail</button>
    </div>
    <div class="field-row">
      <label>WhatsApp tekst</label>
      <textarea id="txt_wa" class="textarea-small">{{ wa_text }}</textarea>
      <button class="btn" type="button" onclick="copyText('txt_wa')">Kopieer WA tekst</button>
    </div>
    <div class="field-row">
      <label>Signal tekst</label>
      <textarea id="txt_signal" class="textarea-small">{{ signal_text }}</textarea>
      <button class="btn" type="button" onclick="copyText('txt_signal')">Kopieer Signal tekst</button>
    </div>
  </div>
</div>
...
function voicaShowProgress(stepText) {
  const overlay = document.getElementById('voica-progress-overlay');
  const label = document.getElementById('voica-progress-text');
  if (!overlay || !label) return;
  label.textContent = (stepText || "") + "Bezig met verwerken...";
  overlay.style.display = 'flex';
}
window.addEventListener('load', function() {
  const formGen = document.getElementById('voica-form-generate');
  const formProc = document.getElementById('voica-form-process');
  if (formGen) formGen.addEventListener('submit', function() { voicaShowProgress("Stap 1: CSR generatie bezig... "); });
  if (formProc) formProc.addEventListener('submit', function() { voicaShowProgress("Stap 2: certificaten verwerken... "); });
});
</script>
"""
...
# =========================
# Render helper: content -> hub layout
# =========================
def _render(
    *,
    error: Optional[str],
    ...
    # Render alleen de VOICA1-content...
    ...
    # ...en laat de Hub layout de pagina afmaken (header/menu/footer + main.css/js)
    return hub_render_page(title="VOICA1 Certificaten", content_html=content_html)


# =========================
# Routes
# =========================
def register_web_routes(app, settings=None, tools=None, voica_cfg=None):
    """
    Hub + Standalone compatible.
    Let op: settings/tools zijn hier niet meer nodig voor layout (die komt uit main_layout).
    """
...
# =========================
# Standalone run (werkt ook vanuit tools/)
# =========================
if __name__ == "__main__":
    app = Flask(__name__)
    # optioneel: laadt config/voica1.json indien aanwezig
    ...
    if cfg_path.exists():
        try:
            ...
            voica_cfg = {}
    register_web_routes(app, voica_cfg=voica_cfg)
